...
var yDomain = round2.map(d => d.firstChoice)
var xDomain = [0,1]
...
var keys = ["Brakey","Bennett","Crafts","No second choice"]

var series = d3.stack().keys(keys)(round2)

console.log(series);

var color = d3.scaleOrdinal()
  .domain(keys)
  .range(["orange","blue","green","#8900c4"])
  //.range(d3.quantize(t => d3.interpolateSpectral(t * 0.8 + 0.1), series.length).reverse())
  .unknown("#ccc")
...
svg.append("g")
  ...
  .join("rect")
    .attr("x", d => xScale(d[0]))//d => x(d[0]))
    .attr("y", d => yScale(d.data.firstChoice)+50)
    .attr("width", d => xScale(d[1]-d[0]))
    .attr("height",yScale.bandwidth());
